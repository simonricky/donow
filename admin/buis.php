...
$limit	= " LIMIT ".$start.", ".$recordperpage;
  ...
  //echo " SELECT  SQL_CALC_FOUND_ROWS  * FROM buiseness WHERE  status='Active' 1=1 ".$condition."  ORDER BY created_at  ";
 $query = mysql_query(" SELECT  SQL_CALC_FOUND_ROWS  * FROM buiseness WHERE  status='Active'  ".$condition."  ORDER BY created_at  ") or die(mysql_error());
    ...
    <div class="InnerSection"><!-- INNER SECTION -->
    	<div class="container-fluid">
        	<div class="row">
                ...
                <div class="col-xs-12">
                	<h2 class="heading">Manage Business</h2>
                    <div class="InnrCont">
                    	<h3 class="subhead" >Businesses</h3>
                    	<div id="msgs" style="display:none;"></div>
                    	<div id="display">
                    	<?php 
                    	$query = " SELECT  SQL_CALC_FOUND_ROWS  *  FROM buiseness  WHERE  status='Active'  ".$condition."  ORDER BY created_at  ".$limit;
                    	...
                    	<div class="ads_row">
						<div class="row">
						<div class="col-sm-3 col-xss-4 col-xs-8"><span class="adimgcont"><img src="../profile/<?php echo $row['logo'];?>" alt="ad1" class="img-responsive" /></span></div>
						<div class="col-sm-2 col-xss-8 col-xs-4 col-sm-push-7 mob_pad0">
						<nav>
						<ul>
						<?php /*<li><a href="update_business_ad.php?get=<?php echo base64_encode($pageno);?>&data=<?php echo base64_encode($row['id']);?>" data-toggle="tooltip" title="Edit Ad" data-id="<?php echo trim($row['id']);?>" class="edit_ad"><i class="fa fa-pencil"></i></a></li>*/?>
						<li><a href="javascript:void(0);" data-toggle="tooltip" title="Delete Business" data-id="<?php echo trim($row['id']);?>" class="delete_bus"><i class="fa fa-times"></i></a></li>
						</ul>
						</nav>
						</div>
						<div class="col-sm-7 col-xs-12 ad_info col-sm-pull-2">
						<h4><?php echo trim($row['buiseness_name']);?></h4>
						<p><?php echo trim($row['city']);?></p>
						<span class="location"><i class="fa fa-map-marker"></i> <?php echo trim($row['address'])." ".$row['city'].", ".$row['state'].", ".$row['country'];?></span>
						<ul class="filteredlist">
						<li><i class="fa fa-mobile"></i> <?php echo trim($row['contact_no']);?></li>
						<li><i class="fa fa-industry"></i> <?php echo $row['australian_buis_num'];//$time= date_getFullTimeDifference(trim($row['open']),trim($row['close']));echo $time['hours']." h : ".$time['minutes']." m";?> </li>
						<?php /*<li><i class="fa fa-bolt"></i> level <?php echo trim($row['activity_level']);?></li>*/?>
						</ul>
						</div>
						</div>
						</div>
