...
if (isset($_GET['data']) && $_GET['data']!="")
{
	$id=base64_decode($_GET['data']);
	 $sql = " SELECT  a.*,at.open as open_time,at.close as close_time  FROM tbl_advertisement as a LEFT JOIN ad_timing as at ON(a.id = at.ad_id) WHERE 1=1 AND a.id='".$id."'  GROUP BY a.id ";
                        	...
                        	<form id="submit_buiseness" enctype="multipart/form-data">
                            	<div class="row">
                                	<div class="col-sm-6 col-xs-12">
                                    	<div class="form-group">
                                        	<label>Title</label>
                                            <input type="text" placeholder="Ad heading" class="form-control" name="title" value="<?php if($data['status']=="success"){echo $data['result'][0]['heading'];}?>"/>
                                        </div>
                                        <div class="form-group">
                                        	<label>Short Description</label>
                                            <input type="text" placeholder="Enter Short Description" class="form-control" name="short_description" value="<?php if($data['status']=="success"){echo $data['result'][0]['short_description'];}?>"/>
                                        </div>
                                        <div class="form-group">
                                        	<label>Location</label>
                                            <input type="text" placeholder="Enter Your Location" class="form-control" name="location" value="<?php if($data['status']=="success"){echo $data['result'][0]['address'];}?>"/>
                                        </div>
                                    </div>
                                    <div class="col-sm-6 col-xs-12">
                                    	<div class="form-group">
                                        	<label>Title</label>
                                            <textarea class="form-control" placeholder="Enter Long Description" name="long_description" ><?php if($data['status']=="success"){echo $data['result'][0]['long_description'];}?></textarea>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="row">
												...
												<select class="form-control custom-select" name="state">
													<option value="">Select State</option>
														...
														while($res = mysql_fetch_assoc($sql) ){
															if($data['status']=="success" && $data['result'][0]['state']==trim($res['code']))
															{
									...
									<div class="col-sm-4 col-xs-12">
										<div class="form-group">
											<label>City</label>
											<input type="text" placeholder="City" class="form-control" name="city" value="<?php if($data['status']=="success"){echo $data['result'][0]['city'];}?>"/>
										</div>
									</div>
									<div class="col-sm-4 col-xs-12">
                                        <div class="form-group">
                                    	...
                                    	<div class="form-group">
                                            ...
                                            <div class="selectcell">
                                               <!--  <div class="cell_left"> -->
												...
												if($data['status']=="success" && $data['result'][0]['open_time']==date('H:i',$halfhour).":00")
												{
													$selected = 'selected="selected"';
												}else
												{
                                               ...
                                               <?php /* <div class="cell_right">
												...
												$end=strtotime('23:30');
												for ($halfhour=$start;$halfhour<=$end;$halfhour=$halfhour+30*60) {
												if($data['status']=="success" && $data['result'][0]['close_time']==date('H:i',$halfhour).":00")
												{
													$selected = 'selected="selected"';
												}else
												{
                                    ...
                                    <div class="col-sm-4 col-xs-12 col-xss-6">
                                    	<div class="form-group">
                                            ...
                                            <div class="input-group">
                                            	...
                                            	<input type="text" placeholder="0.00" class="form-control" name="price" value="<?php if($data['status']=="success"){echo $data['result'][0]['price'];}?>"/>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-sm-4 col-xs-12 col-xss-6">
                                    	<div class="form-group clearfix">
                                            ...
                                            <select class="form-control custom-select" name="select_level">
                                                ...
                                                <?php 
                                                for($i=0;$i<=5;$i++){
                                                	if($data['status']=="success" && $data['result'][0]['activity_level']==$i)
                                                	{
                                                		$selected = 'selected="selected"';
                                                	}else
                                                	{
                            	...
                            	<div class="row">
                                	<div class="col-sm-4 col-xs-12">
                                    	<div class="form-group ImgUpldOtr">
                                            ...
                                            <div class="img_prev" id='preview'><img id="output" src="<?php if($data['status']=="success"){echo "uploads/".$data['result'][0]['image'];}?>"/></div></div>
                                ...
                                <input type="hidden" name="image" id="image" value="<?php if($data['status']=="success"){echo $data['result'][0]['image'];}?>"/>
                                <input type="hidden" name="add_id"  value="<?php if($data['status']=="success"){echo $data['result'][0]['id'];}?>"/>
                                <input type="hidden" name="page_no"  id="page_no" value="<?php if (isset($_GET['get']) && $_GET['get']!=""){echo base64_decode($_GET['get']);}?>"/>
                                 <input type="hidden" name="action" value="update_ad"/>
                                <input type="submit" name="save" value="Update" id="save" class="signin_btn submitbtn1"/>
                                </form>
                        </div>
                    </div>
                </div>
